...
navigation_data = pd.read_csv("/path/to/path_points.csv")  # Ganti path sesuai lokasi file
...
def publish_path():
    ...
    marker.scale.x = 0.05  # ketebalan garis
    marker.color.r = 1.0
    marker.color.g = 0.0
    marker.color.b = 0.0
    marker.color.a = 1.0  # transparansi (1.0 = tidak transparan)

    marker.pose.orientation.w = 1.0  # rotasi default

    for pt in smoothed_path:
        p = Point()
        p.x = pt[0]
        p.y = pt[1]
        p.z = pt[2] if len(pt) > 2 else 0.0
